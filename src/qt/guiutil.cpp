...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Testwage address (e.g. %1)").arg("DCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Testwage: URI
...
{
    // Convert testwage:// to testwage:
    //
    //    Cannot handle this later, because testwage:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open testwage.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Testwage.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Testwage.lnk
...
{
    return GetAutostartDir() / "testwage.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a testwage.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Testwage\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the testwage app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add testwage app to startup item list
