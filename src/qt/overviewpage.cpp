...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::WAGE)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTestwageAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTestwageAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTestwageAmount = strAnonymizeTestwageAmount.remove(strAnonymizeTestwageAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestwageAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTestwageAmount * COIN) nMaxToAnonymize = nAnonymizeTestwageAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTestwageAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTestwageAmount));
        strAnonymizeTestwageAmount = strAnonymizeTestwageAmount.remove(strAnonymizeTestwageAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestwageAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
