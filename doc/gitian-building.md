...
*Setup instructions for a gitian build of TESTWAGE using a Debian VM or physical system.*
...
- [Building TESTWAGE](#building-testwage)
...
Clone the git repositories for testwage and gitian and then checkout the testwage version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/testwage-crypto/testwage.git
cd testwage
...
Follow the instructions in [doc/release-process.md](release-process.md) in the testwage repository
...
To build TESTWAGE (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the testwage repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/testwage/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/testwage-crypto/testwage
...
For example:
```bash
URL=https://github.com/crowning-/testwage.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/testwage-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/testwage-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/testwage-build.assert
```
...
[testwage/gitian.sigs](https://github.com/testwage-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
